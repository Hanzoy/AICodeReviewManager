...
- 管理 API：<http://127.0.0.1:7000>

在管理界面创建项目组后，页面会生成类似下面的单次启动命令：

```bash
npm run start:group -- --manager-url=http://127.0.0.1:7000 --group-id=<group-id> --enroll-token=<token> --data-dir=./data/group-nodes/<group-id>
```
...
```bash
npm run start:group -- --manager-url=http://127.0.0.1:7000 --group-id=<group-id> --name=<group-name> --data-dir=./data/group-nodes/<group-id>
```
...
```bash
npm run start:group -- --reset-identity --manager-url=http://127.0.0.1:7000 --group-id=<group-id> --enroll-token=<new-token> --name=<group-name> --data-dir=./data/group-nodes/<group-id>
```
...
2. 进入“项目管理”，填写项目名称、GitLab Project ID/Path、Clone 地址和默认分支。
3. 创建后复制页面一次性显示的 Webhook URL 与 Secret token。
4. 在 GitLab 项目的 `Settings → Webhooks` 中填写 URL 和 Secret token。
5. Trigger 只勾选 `Merge request events`，保存后发送测试请求。
...
3. HTTPS Clone 使用同一个 Token 通过临时 AskPass 环境认证；SSH Clone 使用该系统账户现有的 SSH Key。
...
5. 使用 DeepSeek、只读工具白名单和 JSON Schema 执行非交互 Review；提示词要求只检查 `origin/{target}` merge-base 到 HEAD 的差异。
6. 解析固定结构的结论、风险等级与 Findings。
7. 调用 GitLab Notes API，将 Markdown Review 结果发布到对应 MR。

DeepSeek API Key 只保存在项目组节点的 AES-256-GCM 加密文件中。执行日志和结构化结果保存在对应任务的 `review-artifacts/{taskId}` 下，便于排查失败。
...
1. 按分支、提交信息关键词和起止日期筛选本地 Commit 历史。
