...
Commit 树根据父 Commit 和分支引用绘制多轨分叉、合并关系。项目组节点会把 Commit 元数据缓存在 `data/group-nodes/{groupId}/cache/commit-graphs`，不缓存代码文件，也不修改仓库。首次访问会建立完整缓存；后续引用变化时先计算当前可达 Commit 顺序，只读取新增 Commit 的元数据，同时删除已不可达提交并重建分支/Tag 引用。一次变化超过 5,000 个新 Commit 时才回退到完整重建。

节点启动时会在 64 MB 文件预算内后台预热最近更新的缓存；运行时采用 LRU 和总 Commit 数限制保留热点项目，分支可达范围与作者列表也只计算一次。项目详情中的“重新检测”只执行轻量本地仓库校验，不访问 GitLab；“同步远端”会同步全部远端分支和 Tag，但不递归拉取 Submodule，并在返回前增量更新 Commit 缓存。“上次同步时间”保存在独立的小型 `.meta.json` 中，远端无变化时不会重写大型缓存文件。同步与该项目的 Review 执行共享同一把 Git 操作锁，避免 Fetch、Checkout 与 AI Review 并发修改仓库。
